...
/* *
 * User function that realizes the 'Store configuration' command. NodeID and bitRate are stored
 * to EEPROM.
 * @param *object NULL
 * @param id New Node ID
 * @param bitRate New bit timing
 */
bool_t storeIDandBitrate(void *object, uint8_t id, uint16_t bitRate);

/* *
 * User function that realizes the 'Configure bit timing' command. It checks whether new bitrate
 * is supported.
 * The function is empty due to the check already made by CANOpenNode function
 * @param *object NULL
 * @param bitRate New bit timing
 */
